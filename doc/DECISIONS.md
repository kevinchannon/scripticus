...
## D44. Version-spec grammar: cargo-style caret + comparators, in `scripticus_schema`

**Decision**: Dependency specs — the string value of a
`[dependencies.packages]` entry and the `@<spec>` of `install pkg@<spec>`
— use an npm/cargo-style grammar, parsed by
`scripticus_schema.version_spec` (contract, D29: it defines what a
dependency declaration *means*, and both client and server interpret it).
The grammar: `^1.2.3` caret ranges with the upper bound from the
left-most non-zero component (`^0.2.3` → `<0.3.0`, `^0.0.3` → `<0.0.4`),
partial carets allowed (`^1`, `^1.2`); comparators `>= > <= < =` with a
comma as intersection (`>=1.2, <2.0.0`), partial operands zero-padded; a
bare *full* version is an exact match, a bare *partial* is rejected as
ambiguous; `*` (or empty) is any release. Prereleases are opt-in: a
prerelease version is selected only by an exact pin equal to it — caret
and comparator ranges never match one. The module also exposes
`select_version(specs, candidates)`, the reusable window primitive (D42):
the highest candidate satisfying every spec, or `None` for an empty
window. It sits beside `semver.py` and reuses its ordering.

**Reason**: The README and CLI already show caret and exact forms, so the
grammar formalises what was implied rather than inventing one. Rejecting
bare partials sidesteps the one real ambiguity (npm reads `1.2` as
`1.2.x`, cargo as `^1.2`) by making the author write `^1.2` or `1.2.0`.
Prerelease opt-in keeps a plain `install foo` from surprising anyone with
a release candidate. The primitive lives in `schema/` because tool
resolution (D43) and the client's `@spec` parsing reuse it, not just the
server's solver.

**Consequences**:
- Good: one spec grammar and one window function, shared across packages,
  tools, and CLI argument parsing.
- Good: no bare-partial ambiguity to document around; the parser rejects
  it with an actionable message.
- Bad: prerelease-opt-in-by-exact-pin is stricter than npm's
  same-core-prerelease rule — a deliberate simplification that a future
  need could revisit.
- Bad: specs are only validated where the parser runs; publish does not
  yet reject a malformed spec at ingest (a later tightening).
