# Roadmap — v1.0.0 (archived)

> **Archived.** This is the historical v1.0.0 roadmap, retained as a record of
> what the internal release scoped and shipped. The v1 surface is complete; the
> single genuinely-unshipped item (editable/dev install) has been carried into
> the live [ROADMAP.md](../ROADMAP.md). Forward-looking work now lives there.
...
- [x] Server-side dependency resolution (D42): `POST /resolve`
      ...
      roots (D52): install sends one, `update` sends its targets as roots and
      drops them from the installed-version preference set so they float while
      the rest stays put.
...
- [x] `update [<pkg>...]` (D52/D53): re-resolve installed remote-provenance
      ...
      rest stays put, so it is conservative — picks the newest compatible
      version, never downgrades, and when a shared constraint holds a target
      back a post-solve diagnostic returns `held_back` so the client names the
      blocking package rather than reporting "up to date"; an unsatisfiable
      request is install's hard error (D17), escaped by widening the target
      set. A version whose command set
...
- [ ] Editable/dev install (`pip install -e` equivalent): shim points at the
      ...
      publish cycle.
- [x] Post-download content-hash verification against the resolved hash
      (`remote_install.py`: every fetched blob's tree hash is checked against
      the resolved `content_hash` before staging, D3/D46).
