"""Registry + installed listing (D49): the dnf-style `list` command — glob
filtering over identity, the installed/available split, dedup and
installed-exclusion in the available section, offline `--installed`, and
graceful degradation when the registry is absent."""
...
def fake_catalog(monkeypatch, per_remote):
    """Route ``GET /search`` (empty query = catalog) by host to ``per_remote``
    (host -> handler). Returns the recorded request list."""
...
def test_glob_with_slash_scopes_by_namespace(home, monkeypatch):
    fake_catalog(
        monkeypatch,
