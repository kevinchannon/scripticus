...
The last post-v1 command has landed too: the `config` command group
(`config remote add/list/remove` and `config tools --install/--escalate`,
`cli.py` over `config.py`'s `add_remote`/`remove_remote`/`save_tools`) manages
`config.toml` directly — D56 replaced the planned git-pull `config install`
(D12) with these explicit setters, orgs onboarding by shipping the commands
rather than a config repo. The repo is a **uv workspace**
...
**Prefer the automated path.** The whole runbook below is encoded in
[scripts/release](scripts/release), which the `tt release` task in
[tasktree.yaml](tasktree.yaml) passes straight through to — when asked to cut a
release, use `tt release` rather than performing the steps by hand (or
`scripts/release`, same arguments, if `tt` is unavailable). Pass a bump per package
...
manual runbook below is what `scripts/release` automates — the prose source of
truth for its behaviour, and the fallback if the automation is unavailable.
